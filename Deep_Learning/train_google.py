...
import pandas as pd
from model import GoogLeNet,ResNet,AlexNet, densenet201,resnet50,resnet152,densenet161
...
#数据集文件夹
datapath = '/home/zh/corn/Dataset2'
vad = 0.4
...
#         train_acc.append(train_l_sum/n)
...
#     data.to_csv('/home/zh/corn/result/D_Net.csv', mode='a', header=False, index=False)
#         # print('epoch %d, loss %.4f, train acc %.3f, time %.1f sec'
#         #       % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, time.time() - start))
#     torch.save(net.state_dict(), '/home/zh/corn/result/G_D_Net.pth')
...
def train(net, train_iter, test_iter, batch_size, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            l0 = loss(logits, y)
            l1 = loss(aux_logits1,y)
            l2 = loss(aux_logits2, y)
            l = l0 + l1 *0.3 + l2*0.3
            optimizer.zero_grad()
            l.backward()
            optimizer.step()
            train_l_sum += l.cpu().item()
            train_acc_sum += (logits.argmax(dim=1) == y).sum().cpu().item()
            n += y.shape[0]
            batch_count += 1
        test_acc , test_loss = evaluate_accuracy(test_iter, net)
        vad_acc.append(test_acc)
        ...
        List = [vad_acc, vad_loss, train_acc, train_loss]
        data = pd.DataFrame([List])
        data.to_csv('/home/zh/corn/result/G_Net.csv', mode='a', header=False, index=False)
        # print('epoch %d, loss %.4f, train acc %.3f, time %.1f sec'
        #       % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, time.time() - start))
    torch.save(net.state_dict(), '/home/zh/corn/result/G_Net.pth')
...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    # plt.legend()
    plt.savefig('/home/zh/corn/result/G_Net.png')
    # plt.show()
...
model = net
model.load_state_dict(torch.load('/home/zh/corn/result/G_Net.pth'))
